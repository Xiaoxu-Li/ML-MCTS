...
"""
Standard MCTS a 2D graphene with B and N doping
...
#n1 = 3  # corresponding to "1" or "B"
#n2 = 6    # corresponding to "2" or "N"   
...
class State():
    ...
    # positions with different occupancies to exchange
    def find_exchange_atom(self):
        pair_set = []
        occupancies = [0, 1, 2]    
        # Generate all unique occupancy pairs (a, b) with a < b
        for i in range(len(occupancies)):
            for j in range(i + 1, len(occupancies)):
                occ_a = occupancies[i]
                occ_b = occupancies[j]            
                elements_a = self.find_elements_with_occupancy(occ_a)
                elements_b = self.find_elements_with_occupancy(occ_b)            
                pair_set.extend([[k1, k2] for k1 in elements_a for k2 in elements_b])
...
if __name__ == "__main__": 
    ...
    data, vector_opt = MCTS(10000, root, 100)
