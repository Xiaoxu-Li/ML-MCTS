...
"""
Multi-level mcts for a 2D graphene with B and N doping
...
# data from Physical Review B 84, 085409 2011.
# double-index parameters
R_list = np.array([[1.95, 1.95, 1.95], [1.95, 2.0, 1.95], [1.95, 1.95, 2.0]])
D_list = np.array([[0.15, 0.10, 0.10], [0.10, 0.10, 0.05], [0.10, 0.05, 0.10]])
A_list = np.array([[1393.6, 1386.78, 1386.78], [1386.78, 40.0520, 1380.0], [1386.78, 1380.0, 128.86866]]) 
lambda_list = np.array([[3.4879, 3.5279, 3.5279], [3.5279, 2.23725, 3.568], [3.5279, 3.568, 2.8293093]]) 
B_list = np.array([[430.0, 339.06891, 387.575152], [339.06891, 43.132016, 340.0], [387.575152, 340.0, 138.77866]]) 
mu_list = np.array([[2.2119, 2.2054, 2.2054], [2.2054, 2.0774982, 2.199], [2.2054, 2.199, 2.6272721]]) 

# single-index parameters
c_list = np.array([38094, 25000, 25000])
d_list = np.array([4.3484, 4.3484, 4.3484])
h_list = np.array([-0.93, -0.89, -0.89])
beta_list = np.array([1.5724e-7, 1.2572e-6, 1.2572e-6])
n_list = np.array([0.727514, 0.727514, 0.727514])
...
total_level = 2 * m + 1  
#n1 = 4  # corresponding to "1" or "B"
#n2 = 4    # corresponding to "2" or "N"   
#n0 = int(n_atom - n1 - n2) # corresponding to "0" or "C"
...
# Define a function to generate hierarchy sets (only for cell index (double-index))
# Note that 2*m levels for cells hierarchy, but 2*m+1 levels in total (including one more for sublattice)
def create_hierarchy_set(n):
    num_level = int(np.log2(n)) * 2  # Calculate total levels of unit cell indices exchange
    hierarchy_set = [None] * num_level  # Initialize a list to store partitions of each level    
    row_indices = np.arange(n)  
    col_indices = np.arange(n)  
    for l in range(num_level):  # Traverse each level
        num_segments = 2 ** (l + 1)  # Calculate the number of segments at current level
        hierarchy_set[l] = [None] * num_segments  # Initialize partition list for the current level     
        num_row = 2 ** (l // 2 + 1)
        num_col = 2 ** (l // 2 + l % 2)
        segment_row_size = int(n / num_row)
        segment_col_size = int(n / num_col)
        for segment in range(num_segments):  # Traverse each segment
            # Because adjacent sets in hierarchy_set need to be paired, we classify the odd and even cases
            if l % 2 == 0:  # Row split level              
                row_start = (segment % num_row) * segment_row_size
                row_end = (segment % num_row + 1) * segment_row_size
                rows = row_indices[row_start:row_end]
                col_start = (segment // num_row) * segment_col_size
                col_end = (segment // num_row + 1) * segment_col_size
                cols = col_indices[col_start:col_end]
            else:  # Column split level            
                col_start = (segment % num_col) * segment_col_size
                col_end = (segment % num_col + 1) * segment_col_size
                cols = col_indices[col_start:col_end]
                row_start = (segment // num_col) * segment_row_size
                row_end = (segment // num_col + 1) * segment_row_size
                rows = row_indices[row_start:row_end]
            row_col_pairs = np.array([[i, j] for i in rows for j in cols])  # Generate all row-column pairs
            hierarchy_set[l][segment] = row_col_pairs  # Store the row-column pairs in hierarchy_set   
    return hierarchy_set 

# Generate hierarchy sets
hierarchy_set = create_hierarchy_set(n)  
...
class State():
    ...
    # Initialize
    def __init__(self, vector, level = 0):
        self.vector = vector
        self.level = level % total_level
    ...
    # Generate a pair set containing all possible exchangeable atom pairs
    # across adjacent segments in the current hierarchy level
    def find_exchange_atom(self):
        pair_set = []
        level = self.level
        occupancies = [0, 1, 2]
        if level == 2 * m:
            # Sublattice exchange: only across sublattices
            for i in range(len(occupancies)):
                for j in range(i + 1, len(occupancies)):
                    occ1 = occupancies[i]
                    occ2 = occupancies[j] 
                    element1 = self.find_elements_with_occupancy(occ1)
                    for index1 in element1:
                        index2 = [index1[0], index1[1], int(1 - index1[2])]
                        if self.vector[index2[0], index2[1], index2[2]] == occ2:
                            pair_set.append([index1, index2])
        else:
            ...
            for i in range(len(occupancies)):
                for j in range(i + 1, len(occupancies)):
                    ...
                    element1 = self.find_elements_with_occupancy(occ1)
                    for index1 in element1:
                        # Find segment index of index1 at this level
                        for segment in range(2 ** (level + 1)):
                            set1 = hierarchy_set[level][segment]
                            if (index1[0], index1[1]) in map(tuple, set1):
                                break
                        # Determine neighboring segment
                        if segment % 2 == 0:
                            neighbor_segment = segment + 1
                        else:
                            neighbor_segment = segment - 1
                        # Search for index2 in the neighbor segment
                        for index2_cell in hierarchy_set[level][neighbor_segment]:
                            for k in range(2):  # sublattice layer
                                index2 = [index2_cell[0], index2_cell[1], k]
                                if self.vector[index2[0], index2[1], index2[2]] == occ2:
                                    pair_set.append([index1, index2])
    ...
    def next_state(self):
        ...
        new_vector = copy.copy(self.vector)
        index1, index2 = pair_set[i]            
        new_vector[index1[0], index1[1], index1[2]], new_vector[index2[0], index2[1], index2[2]] = \
            new_vector[index2[0], index2[1], index2[2]], new_vector[index1[0], index1[1], index1[2]]
        next = State(vector = new_vector, level = self.level + 1) 
    ...
    def __repr__(self):
        s = "level: %d, vector: %s" % (self.level, str(self.vector))
        return s
...
class Node():
    ...
    def __repr__(self):
        s = "level: %d; value: %f; energy: %.16f; children: %d; visits: %d" \
            % (self.level, self.value, self.energy, len(self.children), self.visits)
        return s
...
# whole MCTS process
def MLMCTS(times:int, root, layer_width:int): 
    ...
    for k in range(times):
        ...
        if k % 200 == 0 and k > 1: 
            index = k // 200
            np.save(f"data/config_mlmcts{index}.npy", vector_opt)
...
# Expand a new node
def EXPAND(node):    
    # Set the same vector (no move) as one of children
    if len(node.children) == 0:
        new_vector = copy.copy(node.state.vector)
        new_level = node.state.level + 1
        new_state = State(vector = new_vector, level = new_level)
    else:   
...
if __name__ == "__main__": 
    ...
    output_path = 'data/result_mlmcts.txt'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    init_vector = np.load('config_init_triple.npy')
    root = Node(State(vector = init_vector))
    data, vector_opt = MLMCTS(10000, root, 100)
    # MCTS(times, root, layer_width)

    np.savetxt(output_path, data, fmt=['%d', '%.12f', '%.12f'], delimiter='\t')
    np.save('data/config_mlmcts_final.npy', vector_opt)
