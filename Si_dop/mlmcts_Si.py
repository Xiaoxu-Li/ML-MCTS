...
"""
Multi-level mcts for a 2D graphene with Si doping
...
class State():
    ...
    # Initialize
    def __init__(self, level = 0, vector = VECTOR):
        self.vector = vector
    ...
    # positions to exchange (with restriction)
    # Note that the position is allowed to exchange with that in adjcent (last or next) segment
    def find_exchange_atom(self):
        ...
        if level == 2 * m:
            for j in range(n1):
                ...
                if self.vector[index2[0], index2[1], index2[2]] == occupancy2:
                    pair = [index1, index2]
            ...
            for j in range(n1):
                ...
                for segment in range(2 ** (level + 1)):
                    ...
                    set1_set = set(map(tuple, set1))
                    if (index1[0], index1[1]) in set1_set:
                        break
                if segment % 2 == 0:
                    index_set2 = segment + 1
                else:
                    index_set2 = segment - 1
                for i in range(2 ** (2 * m - level - 1)):
                    index2_cell = hierarchy_set[level][index_set2][i]
                    for k in range(2):
    ...
    def next_state(self):
        ...
        next = State(level = self.level + 1, vector = new_vector) 
...
def EXPAND(node):    
    ...
    if len(node.children) == 0:
        ...
        new_state = State(level = new_level, vector = new_vector)
    else:   
...
if __name__ == "__main__": 
    ...
    data, vector_opt = MLMCTS(10000, root, 10)
