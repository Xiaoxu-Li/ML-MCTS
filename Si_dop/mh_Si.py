...
"""
Metropolis for a 2D graphene with Si doping
...
Begin the Metropolis sampling,n is the number of our sample, 
c is the concentration of zero and N is the number of Monte-Carlo times.
"""
...
def metropolis_algorithm(n, N, initial_structure):
        ...
        for dy in range(-n + 1, n):    
            ...
            for sub_idx1 in range(2):
                for sub_idx2 in range(2):
                    ...
                    x_translated = x_translations[:, None] * mcts_final.x_basis[0] \
                        + y_translations[None, :] * mcts_final.y_basis[0] \
                        + mcts_final.sub_basis[sub_idx1, 0] - mcts_final.sub_basis[sub_idx2, 0]
                    y_translated = x_translations[:, None] * mcts_final.x_basis[1] \
                        + y_translations[None, :] * mcts_final.y_basis[1] \
                        + mcts_final.sub_basis[sub_idx1, 1] - mcts_final.sub_basis[sub_idx2, 1]                    
    ...
    for i in range(n):
        for j in range(n):
            for k in range(2):
                ...
                coordinate_list[index_1d, :] = i * mcts_final.x_basis \
                    + j * mcts_final.y_basis + mcts_final.sub_basis[k, :]
                

    # Create a set to store the indices of neighbouring atoms
    radius_max = np.max(mcts_final.R_list + mcts_final.D_list)
    num_neighbour = 0
    ...
    def Bond_order(structure, index1, index2):
        ...
        beta = mcts_final.beta_list[occupancy1]
        n_ = mcts_final.n_list[occupancy1]
        sum = 0.0
        ...
        for index_1d in range(n_atom):                   
            if index_1d != index1_1d and index_1d != index2_1d:
                ...
                R = mcts_final.R_list[occupancy1, occupancy3]
                D = mcts_final.D_list[occupancy1, occupancy3]
                if r <= R + D:
                    sum += mcts_final.fc(r, R, D) * mcts_final.Angle(vec1, vec2, occupancy1)
    ...
    def energy(structure):
        ...
        for index1_1d in range(n_atom):
            ...
            for j in range(num_neighbour):
                ...
                R = mcts_final.R_list[occupancy1, occupancy2]
                D = mcts_final.D_list[occupancy1, occupancy2]
                if r <= R + D:
                    E += mcts_final.fc(r, R, D) * (mcts_final.fR(r, occupancy1, occupancy2) + \
                        mcts_final.fA(r, occupancy1, occupancy2))
                    #E += mcts_final.fc(r, R, D) * (mcts_final.fR(r, occupancy1, occupancy2) + \
                    #   Bond_order(structure, index1, index2) * mcts_final.fA(r, occupancy1, occupancy2))
        E = E / (2 * n_atom)
    ...
    while(N > 0):
        ...
        # Exchange a pair of indices
        set1 = find_elements_with_occupancy(data_structure, 0)
        set2 = find_elements_with_occupancy(data_structure, 1)
...
if __name__ == "__main__": 
    ...
    pr = cProfile.Profile()
    pr.enable()  
    initial_structure = np.load('config_init.npy')
